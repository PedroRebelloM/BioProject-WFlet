...
def verificarEmail(email: str) -> bool:
    ...
    )  # r'aceita lums e letras + @ com nums e letras + . com letras e nums'


def Entrar(
    campoUsuario: ft.TextField, campoSenha: ft.TextField, area: ft.Text, page: ft.Page
):
...
def EscolherArquivoBancoDeDados(
    e: ft.FilePickerResultEvent, qddArquivos, page: ft.Page
):
...
def atualizarListaArquivos(linha: ft.Column, user_id):
    ...
    arquivos = db.files.find(
        {"user_id": user_id}
    )  # Requisição de acordo com o Id do usuário no banco de dados
    ...
    for arquivo in arquivos:
        ...
        if len(botoes) == 3:
            linhaBotoes = ft.Row(  # Linha para os trios
                controls=botoes, alignment=ft.MainAxisAlignment.START, spacing=10
            )
    ...
    linha.controls.extend(
        linhasDeBotoes
    )  # Adicionando as linhas ao controle de linha (extend por ser mais de um objeto)
...
def downloadArquivo(file_path, container: ft.Container):
    ...
    if arquivo:
        ...
        savePath = os.path.join(
            os.path.expanduser("~"), "Downloads", os.path.basename(file_path)
        )
        ...
        try:
            ...
            snack_bar = ft.SnackBar(
                content=ft.Text(f"Erro ao baixar {file_path}: {e}"), duration=2000
            )
...
def buscarNoNCBI(page: ft.Page, bancoSelecionado, campoTexto, containerBD):
    ...
    if not termo:
        ...
        containerBD.content.controls.append(
            ft.Text(
                value="Por favor, insira um termo de pesquisa.",
                color="black",
                size=12,
                weight=ft.FontWeight.BOLD,
            )
    ...
    def buscarIds(termo, retstart=0):
        ...
        try:
            ...
            if (
                "esearchresult" in resultadoPesquisa
                and "idlist" in resultadoPesquisa["esearchresult"]
            ):
                count = int(
                    resultadoPesquisa["esearchresult"]["count"]
                )  # Converte count para inteiro
            ...
            containerBD.content.controls.append(
                ft.Text(
                    value=f"Erro ao buscar resultados: {str(e)}",
                    color="black",
                    size=12,
                    weight=ft.FontWeight.BOLD,
                )
    ...
    parametroSumario = {"db": banco, "id": ids, "retmode": "json"}
    ...
    try:
        ...
        containerBD.content.controls.append(
            ft.Text(
                value=f"Erro ao buscar detalhes dos resultados: {str(e)}",
                color="black",
                size=12,
                weight=ft.FontWeight.BOLD,
            )
        ...
        containerBD.content.controls.append(
            ft.Text(
                value=f"Erro ao processar resultados: {str(e)}",
                color="black",
                size=12,
                weight=ft.FontWeight.BOLD,
            )
    ...
    if "result" not in resultado:
        ...
        containerBD.content.controls.append(
            ft.Text(
                value="Erro ao buscar detalhes dos resultados.",
                color="black",
                size=12,
                weight=ft.FontWeight.BOLD,
            )
    ...
    def copiarId(uid):
        ...
        page.snack_bar = ft.SnackBar(
            ft.Text("ID copiado para a área de transferência!")
        )
        page.snack_bar.open = True
        page.update()
    ...
    def baixarArquivo(uid):
        ...
        resposta.raise_for_status()
        pastaDownload = os.path.join(
            os.path.expanduser("~"), "Downloads", f"{uid}.fasta"
        )
                        ...
                        ft.Row(
                            [
                                ft.ElevatedButton(
                                    ...
                                    on_click=lambda e, url=f"https://www.ncbi.nlm.nih.gov/sviewer/viewer.fcgi?id={uid}&db={banco}&report=fasta&retmode=text": downloadUrl(
                                        url
                                    ),
